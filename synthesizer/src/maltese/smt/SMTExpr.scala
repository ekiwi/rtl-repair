...
sealed trait BVBinaryExpr extends BVExpr {
  ...
  def reapply(nA: BVExpr, nB: BVExpr): BVExpr
}
...
object BVAnd {
  ...
  def apply(exprs: Seq[BVExpr]): BVExpr = {
    ...
    nonTriviallyTrue.distinct match {
      case Seq()    => True()
      case Seq(one) => one
      case h :: t   => t.foldLeft(h)(BVOp(Op.And, _, _))
    }
  }
  def unapply(op: BVOp): Option[(BVExpr, BVExpr)] = if (op.op == Op.And) { Some((op.a, op.b)) }
  else None
}
object BVOr {
  def apply(a: BVExpr, b: BVExpr): BVExpr = {
  ...
  def apply(exprs: List[BVExpr]): BVExpr = {
    ...
    nonTriviallyFalse.distinct match {
      case Seq()    => False()
      case Seq(one) => one
      case h :: t   => t.foldLeft(h)(BVOp(Op.Or, _, _))
    }
  }
  def unapply(op: BVOp): Option[(BVExpr, BVExpr)] = if (op.op == Op.Or) { Some((op.a, op.b)) }
  else None
}
