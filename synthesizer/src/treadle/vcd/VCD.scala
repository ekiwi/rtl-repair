...
object VCD {
  ...
  class WordIterator(fileName: os.Path) extends Iterator[String] {
    val lines: Iterator[String] = os.read.lines(fileName).iterator
  ...
  def read(
          ...
          case EndSection() =>
            scopeBuffer.toString() match {
              case ScopedModule(kind, moduleName) =>
                if (kind != "module") {
                  debug(s"unsupported scope type ${scopeBuffer.toString()} at line ${words.currentLineNumber}")
                }
                addScope(moduleName)
              case _ =>
                warn(s"unknown scope format ${scopeBuffer.toString()} at line ${words.currentLineNumber}")
            }
            scopeBuffer.clear()
          case text =>
    ...
    def addVar(s: String): Unit = {
      s match {
            ...
            case Some(varName) =>
              if (desiredScopeFound) {
                ...
                if (!wireIdToWire.contains(idString)) {
                  ...
                  debug(s"AddVar $wire at line ${words.currentLineNumber}")
                  currentScope.foreach(_.wires += wire)
                } else {
                  debug(s"AddVar aliased wire $wire at line ${words.currentLineNumber}")
                  aliasedWires.getOrElseUpdate(idString, new mutable.HashSet) += wire
                  currentScope.foreach(_.wires += wire)
                }
              } else {
                debug(s"Ignore var ${scopePathString(currentScope)}$varName at line ${words.currentLineNumber}")
        ...
        case _ =>
          warn(s"Could not parse var $s at line ${words.currentLineNumber}")
      }
    }
    ...
    def processDump(): Unit = {
      while (words.hasNext) {
        ...
        // debug(s"Process dump $nextWord at line ${words.currentLineNumber}")
        nextWord match {
          ...
          case TimeStamp(timeValue) =>
            ...
            debug(s"current time now $currentTime at line ${words.currentLineNumber}")
          case EndSection() =>
            debug(s"end of dump at line ${words.currentLineNumber}")
          case _ =>
        }
      }
    }
      ...
      if (words.hasNext) {
        ...
        nextWord match {
          case SectionHeader(sectionHeader) =>
            debug(s"processing section header $sectionHeader at line ${words.currentLineNumber}")
            sectionHeader match {
          ...
          case _ =>
            processDump()
            debug("skipping at line ${words.currentLineNumber}")
        }
        processSections()
      }
    }
    ...
    if (scopeRoot.isEmpty) {
      throw new RuntimeException(s"Error: No start scope found, desired StartScope is $startScope")
    }
...
case class VCD(
    ...
    wireName.split("""\.""").reverse.toList match {
      case name :: reversedScopes =>
        findScope(scopeRoot, reversedScopes.reverse) match {
          ...
          case None =>
            throw new RuntimeException(s"Could not find scope for $wireName")
        }
      case Nil =>
        throw new RuntimeException(s"Can not parse found wire $wireName")
    }
  }
  ...
  def wireChanged(wireName: String, value: BigInt, width: Int = 1): Boolean = {
    ...
    // info(f"vcd-change time $timeStamp%6d value $value%6d wire $wireName")
    if (!wires.contains(wireName)) {
