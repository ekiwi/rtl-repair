...
object TransitionSystem {
  ...
  def hasQuantifier(sys: TransitionSystem): Boolean =
    systemExpressions(sys).exists(hasQuantifier)
  private def hasQuantifier(e: SMTExpr): Boolean = e match {
    case _: BVForall => true
    case _ => e.children.exists(hasQuantifier)
  }
}
