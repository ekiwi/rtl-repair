...
def parse_args() -> Config:
    ...
    parser.add_argument('--osdd-toml', help='Path to osdd.toml generated by calc_osdd.py.', required=False)
    parser.add_argument('--baseline-toml', help='Path to baseline_results.toml generated by generate_vcd_traces.py.', required=False)
    ...
    if conf.baseline_toml is not None:
            ...
            for template in ['replace_literals', 'assign_const', 'add_inversions']:
                ...
                if num_sols == 0:
                    ...
                    if check_passes:
                        ...
                        status = Fail
                row += [f"{changes}{status}", f"{format_time_s(time)}"]
