...
fn find_missing_inputs(
    ...
    for (input, _) in sys.get_signals(|s| s.kind == SignalKind::Input) {
        ...
        if !included {
            ...
            let is_anonymous = name.starts_with("input");
            if verbose && !is_anonymous {
            ...
            out.push(IOInfo {
                ...
                is_input: true,
                name: name.to_string(),
                is_anonymous_input: is_anonymous,
            })
        }
    }
    out
    ...
    while pos < mmap.len() {
        ...
        if !tokens.is_empty() {
            for io in ios.iter() {
                ...
                if is_missing {
                    if io.is_anonymous_input {
                        // anonymous inputs generated by yosys are always set to zero
                        data.push(0);
                    } else {
        ...
        if let Some(signal_ref) = name_to_ref.get(name.as_ref()) {
            ...
            if matches!(signal.kind, SignalKind::Input | SignalKind::Output) {
                ...
                out.push(IOInfo {
                    ...
                    name: name.to_string(),
                    is_anonymous_input: false, // anonymous inputs are not part of the testbench
                })
            } else if verbose {
